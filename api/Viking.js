...
class Viking {
    ...
    getActionPosition() {
        ...
        function boundryMin(value, min) {
            return value <= min ? min : value;
        }
        function boundryMax(value, max) {
        ...
        if (p.x >= -1 && p.x <= 1 && p.y >= -1 && p.y <= 1) {
            ...
            position.x = boundryMin(position.x, 0);
            position.x = boundryMax(position.x, mapSizeX);

            position.y = boundryMin(position.y, 0);
            position.y = boundryMax(position.y, mapSizeY);
        } else {
